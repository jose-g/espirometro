...
public class AccesoActivity extends AppCompatActivity implements
    ...
    private void initUI() {
        oldColors =  tvDepartamento.getTextColors();
    ...
    @OnCheckedChanged(R.id.cbConglomerado)
    void onConglomeradoSelected(CompoundButton buttonView, boolean isChecked) {
        if(isChecked) {
            llConglomerado.setVisibility(View.VISIBLE);
        } else {
            llConglomerado.setVisibility(View.GONE);

            tvDepartamento.setText("Seleccionar");
            tvDepartamento.setTextColor(oldColors);
    ...
    @OnClick(R.id.rlTipoConglomerado)
    public void onClicTipConglomerado(View view) {
        if(id_distrito.isEmpty()) {
            Toasty.info(getApplicationContext(), "Seleccione distrito", Toasty.LENGTH_SHORT).show();
            return;
        }
        progressDialog = Utils.showProgressDialog(AccesoActivity.this, "Cargando...");
        progressDialog.show();
        MSerGetTokens(1);
    }

    @OnClick(R.id.rlConglomerado)
    public void onClicConglomerado(View view) {
        if(id_tipo_conglomerado.isEmpty()) {
            Toasty.info(getApplicationContext(), "Seleccione tipo conglomerado", Toasty.LENGTH_SHORT).show();
            return;
        }
        progressDialog = Utils.showProgressDialog(AccesoActivity.this, "Cargando...");
        progressDialog.show();
    ...
    public void onClicIngresar(View view) {
        boolean airplane_mode = Settings.System.getInt(getApplicationContext().getContentResolver(), Settings.Global.AIRPLANE_MODE_ON, 0) != 0;
        ...
        if(tietUsuario.getText().toString().isEmpty()) {
            Toasty.info(getApplicationContext(), "Ingrese usuario", Toasty.LENGTH_SHORT).show();
            return;
        }

        if(tietClave.getText().toString().isEmpty()) {
            Toasty.info(getApplicationContext(), "Ingrese clave", Toasty.LENGTH_SHORT).show();
            return;
        }

        if(cbConglomerado.isChecked()) {
            ...
            if(id_conglomerado.isEmpty()) {
                Toasty.info(getApplicationContext(), "Seleccione conglomerado", Toasty.LENGTH_SHORT).show();
                return;
            }
